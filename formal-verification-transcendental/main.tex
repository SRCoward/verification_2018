...
Like the Gappa solution, MetiTarski required further assistance with some basic rewriting for one of the inequalities. We can take these separate conjectures and AND them together, since they are all subject to the same constraints. The full proof script is given in Appendix \ref{sine_appendix}. This problem was provable using MetiTarski, with a runtime of 115 seconds on an Intel I7-3517U CPU.

We will briefly digress, as these experiments also highlighted some interesting MetiTarski behaviour. MetiTarski supports 3 different RCF solvers: Z3 \cite{de2008z3}, QEPCAD B \cite{brown2003qepcad} and Mathematica \cite{Mathematica}. Only by using Mathematica as the underlying RCF solver, were we able to prove the results. In fact, neither Z3 nor QEPCAD B could prove the significantly easier problem,
\begin{equation}
...
We have also identified a bug in an algorithm that was close to production. Most interesting are the performance results, which suggest that this technique will be viable in the future, when higher precision algorithms are developed. A software implementation of the sine function was also successfully verified. This highlights the potentially broad scope of this approach, although more complex software algorithms may not be amenable. 
...
We show the MetiTarski problem from \S \ref{gappa}, which uses 3 variables and the sin-extended axioms. It is comprised of four conjectures under the same constraints, which we AND together. 
\begin{itemize}
